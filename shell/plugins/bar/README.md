...
The bar is configured directly on the bar itself: drag empty bar space (or click-and-hold) to move the bar to another screen edge, double-left-click empty center-bar space to toggle transparency, and drag widgets to reorder them. Set `bar.doubleClickTogglesTransparency` to `false` in `shell.json` to disable the double-click gesture without disabling transparency or the `omarchy bar transparent` command. The gesture remains enabled when the option is omitted. The `omarchy bar position`, `omarchy bar transparent`, `omarchy bar move`, and `omarchy bar set` commands do the same from scripts. Enable or disable widgets with `omarchy plugin enable` and `omarchy plugin disable` (widget ids come from `omarchy plugin list`).
